...
// Initialize an empty variable to hold our HTML response.
$response_html = '';
...
if ($conn->connect_error) {
    ...
    // --- 2. DATA RETRIEVAL ---
    // Using ?? '' prevents 'undefined key' errors
    ...
    // --- 3. ATTEMPT TO INSERT THE BLOOD REQUEST ---
    // The column names in this query MUST EXACTLY MATCH your database table.
    // Check for typos like 'bloodType' vs 'blood_type'.
    $insertSql = "INSERT INTO blood_requests (hospitalEmail, bloodType, urgency, quantity, notes, createdAt, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $insertStmt = $conn->prepare($insertSql);

    // This checks if the SQL query itself is valid.
    if ($insertStmt === false) {
        ...
        if ($insertStmt->execute()) {
            ...
            // --- 4. GET HOSPITAL DETAILS FOR THE MESSAGE ---
            $hospitalName = "The Hospital";
            $hospitalLocation = "an unknown location";
            $hospitalContactEmail = $hospitalEmail; // Use submitted email as a fallback

            // Get full details from 'hospitals' table. Selecting specific columns is better than SELECT *.
            ...
            // --- 5. FIND DONORS AND CREATE THE ENHANCED MESSAGE ---
            ...
            if ($result->num_rows > 0) {
                ...
                // YOUR NEW MESSAGE: Now includes the hospital's location AND contact email.
            ...
            // --- THE ULTIMATE DEBUGGING BLOCK ---
            // If the insert failed, this will show the exact reason.
...
<!-- Your full HTML and CSS page structure remains the same -->
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Request Status</title>
    <!-- All your <style> tags go here -->
    <style>
...
<body>
    <div class="container">
        ...
        <!-- Because this opens in a new tab, a "Close" button is more user-friendly -->
